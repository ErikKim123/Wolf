...
// Design Ref: §3.2 상품 상태 전이 — 파트너 권한 한정.
// 파트너는 '초안 → 승인요청(pending)' 까지만. 승인(pending→active)은 운영자 전용.
export const PARTNER_PRODUCT_TRANSITIONS: Record<string, { to: string; label: string }[]> = {
  draft: [{ to: 'pending', label: '승인요청' }],
  pending: [{ to: 'draft', label: '요청취소' }],
...
export const productsConfig: ResourceConfig<ProductRow> = {
  ...
  createDefaults: { product_type: 'physical', status: 'draft', is_partner_product: true },
