// 행사패스관리 (파트너) — 내 티켓 상품만(seller_id 범위) + 생성/수정(카테고리 select + 이벤트 콘텐츠)
...
export default function EventPassesPage() {
  ...
  // 신규 생성 시 seller_id 를 현재 파트너로 주입.
  const config = useMemo(
    () => ({
      ...eventPassesConfig,
      createDefaults: { ...eventPassesConfig.createDefaults, seller_id: userId },
    }),
    [userId],
  );
